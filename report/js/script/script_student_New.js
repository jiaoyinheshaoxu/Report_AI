...
function fullScoreJurge(fullId){
    ...
    var fs_goodword=document.getElementById("fs_goodword12");
    var fs_problem=document.getElementById("fs_problem2");
    var fs_highPoint=document.getElementById("fs_highPoint2");
    var fs_weekPoint=document.getElementById("fs_weekPoint2");
    var errorTest=document.getElementById("errorTest2");
    var strongNum=document.getElementById("strongNum2");
    if(fullId){
...
function yemianJIAzai(){
    var delay=document.getElementById("delay2");
...
function delayload(contentType,perId){
    ...
    function judgeJurisdiction(){
        ...
        var jiaofei=document.getElementById("jiaofei2");
        var judge=document.getElementById("judge2");
    ...
    function deelwithFace() {
        ...
        var personalInfo=document.getElementById("personalInfo2");
        var oh1=personalInfo.querySelectorAll("h1");
        oh1[0].querySelector("span").innerHTML=PersonReportData.cover.studentName+"同学";
        oh1[2].querySelector("span").innerHTML="（"+PersonReportData.cover.subjectName+"）";
        //学号，班级
        var faceDetail = document.getElementsByClassName("faceDetail2");
//      faceDetail[0].innerHTML = PersonReportData.cover.code;
    ...
    function deelWithYemei(){
        ...
        for(var i=0;i<yemei.length;i++){
            yemei[i].innerHTML=PersonReportData.cover.studentName+"同学"+"期末考试分析报告"+"("+ PersonReportData.cover.subjectName+ ")"
        }
    }
    try {
        deelWithYemei()
    ...
    function deelwithPage2() {
        //var judgeProblem=document.getElementById("judgeProblem2");
        //if(PersonReportData.weakPointName.length>0||PersonReportData.weakSubjectAbility.length){
        //    judgeProblem.style.display="block";
        //}else{
        //    judgeProblem.style.display="none";
        //}

        //1、单体数据；
        function anserwerDate() {
            //
            var testDate=document.getElementById("testDate2");
            var testSPan=testDate.querySelectorAll("span");

            testSPan[0].innerHTML="<b class='testDateSpan2'>"+deelwithscore(PersonReportData.questionData.studentScore,1)+"</b>  分";


            testSPan[1].innerHTML=",班级排名第 <b class='testDateSpan2'>"+PersonReportData.questionData.classSort+"</b> 名 ,";

            testSPan[2].innerHTML="年级排名第 <b class='testDateSpan2'>"+PersonReportData.questionData.gradeRange+"</b> 名";
             //按个人方式出的报告

            //app
            var testDate_app=document.getElementById("testDate_app2");
            if(PersonReportData.questionData){
                var ob=testDate_app.querySelectorAll("span");
                ob[0].innerHTML=PersonReportData.questionData.totalScore;
                ob[1].innerHTML=PersonReportData.questionData.questionNum;
                ob[2].innerHTML=PersonReportData.questionData.unitNum;
                ob[3].innerHTML=PersonReportData.questionData.pointNum;
            }else{
        ...
        function problem(){
            function findProblem() {
                ...
                var oSpanProblem = document.getElementById("findProblem2");
                var judgefindProblem=document.getElementById("judgefindProblem2");
                var ary = [];
                var ospan1 = null;
                var ospan2 = null;
                var length=0;
            ...
            function findabilitiy(){
                var oSpanAbility=document.getElementById("findabilitiy2");
                var judgefindProblem=document.getElementById("judgefindabilitiy2");
...
//_______________________________________________________________________________________学科能力图谱结构变化新旧判断
        ...
        function learnSuggestion(){
            var olearnSuggestion=document.getElementById("learnSuggestion2");
            var judgelearnSuggestion=document.getElementById("judgelearnSuggestion2");
            var oli=null;
    ...
    function weektest(){
        //score
        var oScore=document.getElementById("weekTestScore2");
        oScore.innerHTML=deelwithscore(PersonReportData.weekPaperScore.studentScore,1);
        //rank
        var oRank=document.getElementsByClassName("weekTestRank2");
        ...
        oRank[3].innerHTML="第"+deelwithscore(PersonReportData.weekPaperScore.gradeRange)+"名";
    ...
    try {
        weektest();
        if(perId){
            document.getElementById('pId_one2').style.display="none";
        }
    } catch(err){
        //在此处理错误
    }
    ...
    try {
        ...
        if(perId){
            document.getElementById('pId_two2').style.display="none";
        }
...
//        chart.series[0].setData(JSONpart2_Survey);
    ...
    function deelknowledge() {
        ...
        var tableFragment = document.createDocumentFragment();
        var knowledge = document.getElementById("knowledgeName2");
    ...
    function deelknowledge_high() {
        ...
        var tableFragment = document.createDocumentFragment();
        var knowledge = document.getElementById("knowledgeName12");
        var knowledgeBox = null;
        var knowledgename = null;
        var currentMaster = null;
        var averageMaster = null;
...
//      var studata=PersonReportData.subjectPowerGraph.studentScore;
        (function () {
            $('#PART3_table_ability_Map2').highcharts({
        ...
        function findabilitiy2(){
            //新课标界定
            var totalAbility=document.getElementsByClassName("totalAbility12");
            ...
            var totalAbility2=document.getElementById("totalAbility22");
            var oP_oSpanAbility2=document.getElementById("oP_oSpanAbility22");
            var ary = [];
            var ospan1 = null;
            var ospan2 = null;
            ospan1 = document.createElement("span");
...
//                            cxt.clearRect(0, 0, circleWidth, circleHeight);
    ...
    function erranswerList(){
        ...
        var errquestionList=document.getElementById("errquestionList2");
...
//        odiv.appendChild(pointDiv);
    ...
    function getIntensiveExercise(){
        ...
        var IntensiveExercise=document.getElementById("IntensiveExercise2");
        //判断强化练习存不存在
...
//        errquestionList.innerHTML=ClassReportData.errquestionList[0].text;
        ...
        for(var k=0;k<datalist.length;k++) {
            if(datalist[k].qtype==4){
                outDiv_jianda.push(datalist[k])
            }else if(datalist[k].qtype==3){
                outDiv_tiankong.push(datalist[k])
            }else if(datalist[k].qtype==2){
                outDiv_duoxunze.push(datalist[k])
            }else if(datalist[k].qtype==1){
                outDiv_xunze.push(datalist[k])
            }
        }
        ...
        var solutionDiv=null;
        var CommentDiv=null;
        if(outDiv_xunze.length>0){
            var oh2=document.createElement("h2");
            oh2.innerHTML="单项选择题"+"(共"+outDiv_xunze.length+"道)";
            fragment.appendChild(oh2);
            for(var i=0;i<outDiv_xunze.length;i++){
                ob = null;
                odiv = null;
                pointDiv=null;
                AnalysisDiv=null;
                solutionDiv=null;
                CommentDiv=null;
                outDiv_xunze[i].order = "题" + (i + 1) + ":";
                odiv = document.createElement("div");
                odiv.className="exercisesStyle";
                ob = document.createElement("b");
                ob.innerHTML = outDiv_xunze[i].order;
                //添加题号
                odiv.appendChild(ob);
                var oh4=document.createElement("h4");
                oh4.innerHTML =formatQuestionText1(outDiv_xunze[i].text) ;
                //添加题目
                odiv.appendChild(oh4);
                for (var j = 0; j < outDiv_xunze[i].answers.length; j++) {
                    var option= document.createElement("div");
                    if(outDiv_xunze[i].qtype==1||outDiv_xunze[i].qtype==2){
                        option.innerHTML = outDiv_xunze[i].answers[j].holeAnwser;
                        option.className="ansSelectLIst"
        ...
        if(outDiv_duoxunze.length>0){
            var oh2=document.createElement("h2");
            oh2.innerHTML="多项选择题"+"(共"+outDiv_duoxunze.length+"道)";
            fragment.appendChild(oh2);
            for(var i=0;i<outDiv_duoxunze.length;i++){
                ob = null;
                odiv = null;
                pointDiv=null;
                AnalysisDiv=null;
                solutionDiv=null;
                CommentDiv=null;
                outDiv_duoxunze[i].order = "题" + (i + 1) + ":";
                odiv = document.createElement("div");
                odiv.className="exercisesStyle";
                ob = document.createElement("b");
                ob.innerHTML = outDiv_duoxunze[i].order;
                //添加题号
                odiv.appendChild(ob);
                var oh4=document.createElement("h4");
                oh4.innerHTML =formatQuestionText1(outDiv_duoxunze[i].text) ;
                //添加题目
                odiv.appendChild(oh4);
                for (var j = 0; j < outDiv_duoxunze[i].answers.length; j++) {
                    var option= document.createElement("div");
                    if(outDiv_duoxunze[i].qtype==1||outDiv_duoxunze[i].qtype==2){
                        option.innerHTML = outDiv_duoxunze[i].answers[j].holeAnwser;
                        option.className="ansSelectLIst"
        ...
        if(outDiv_tiankong.length>0){
            var oh2=document.createElement("h2");
            oh2.innerHTML="填空题"+"(共"+outDiv_tiankong.length+"道)";
            fragment.appendChild(oh2);
            for(var i=0;i<outDiv_tiankong.length;i++){
                ob = null;
                odiv = null;
                pointDiv=null;
                AnalysisDiv=null;
                solutionDiv=null;
                CommentDiv=null;
                outDiv_tiankong[i].order = "题" + (i + 1) + ":";
                odiv = document.createElement("div");
                odiv.className="exercisesStyle";
                ob = document.createElement("b");
                ob.innerHTML = outDiv_tiankong[i].order;
                //添加题号
                odiv.appendChild(ob);
                var oh4=document.createElement("h4");
                oh4.innerHTML =formatQuestionText1(outDiv_tiankong[i].text) ;
                //添加题目
                odiv.appendChild(oh4);
                for (var j = 0; j < outDiv_tiankong[i].answers.length; j++) {
                    var option= document.createElement("div");
                    if(outDiv_tiankong[i].qtype==1||outDiv_tiankong[i].qtype==2){
                        option.innerHTML = outDiv_tiankong[i].answers[j].holeAnwser;
                        option.className="ansSelectLIst"
        ...
        if(outDiv_jianda.length>0){
            var oh2=document.createElement("h2");
            oh2.innerHTML="简答题"+"(共"+outDiv_jianda.length+"道)";
            fragment.appendChild(oh2);
            for(var i=0;i<outDiv_jianda.length;i++){
                ob = null;
                odiv = null;
                pointDiv=null;
                AnalysisDiv=null;
                solutionDiv=null;
                CommentDiv=null;
                outDiv_jianda[i].order = "题" + (i + 1) + ":";
                odiv = document.createElement("div");
                odiv.className="exercisesStyle";
                ob = document.createElement("b");
                ob.innerHTML = outDiv_jianda[i].order;
                //添加题号
                odiv.appendChild(ob);
                var oh4=document.createElement("h4");
                oh4.innerHTML =formatQuestionText1(outDiv_jianda[i].text) ;
                //添加题目
                odiv.appendChild(oh4);
                for (var j = 0; j < outDiv_jianda[i].answers.length; j++) {
                    var option= document.createElement("div");
                    if(outDiv_jianda[i].qtype==1||outDiv_jianda[i].qtype==2){
                        option.innerHTML = outDiv_jianda[i].answers[j].holeAnwser;
                        option.className="ansSelectLIst"
                ...
                option2.innerHTML="解：";
                option2.style.height="200px";
                option2.style.width="100%";
                odiv.appendChild(option2);
                fragment.appendChild(odiv);
            }
        }
//
        ...
        IntensiveExercise.style.display="block";

        //处理强化练习二维码图片;
        var S_practice=document.getElementById("S_practice2");
        var s_word=document.getElementById("s_word2");
...
var loadPdf=document.getElementById("loadPdf2");
