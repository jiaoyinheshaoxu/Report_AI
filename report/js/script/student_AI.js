...
function checkGrade() {
  if (reportType == 2) {
    ...
    //摘要
    (function () {
      $('#testDate>span:nth-child(1)').html('<b class="testDateSpan">' + deelwithscore(PersonReportData.questionData.studentScore, 1) + '</b>');
      $('#testDate>span:nth-child(2)').html(",班级排名第" + '<b class="testDateSpan">' + PersonReportData.questionData.classSort + '</b>' + "名");
      $('#testDate>span:nth-child(3)').html('');
    })();
    //一、本次考试分析
    try {
      (function () {
        $('.averageGShow').hide();
        $('.averageCShow>div>div:nth-child(2)').html('<span style="font-size: 20px;"> 班级得分：  </span><span class="weekTestRank">' + deelwithscore(PersonReportData.weekPaperScore.classAvg, 1) + "分" + '</span>');
        $('.averageCShow>div>div:nth-child(3)').html('<span style="font-size: 20px;"> 班级排名：  </span><span class="weekTestRank">第' + deelwithscore(PersonReportData.weekPaperScore.classSort, 1) + "名" + '</span>')
      })();
    } catch (err) {
    }
    ...
    try {
      (function () {
        ...
        $('#tableMonth').html('');
        $('#tableMonth').append('<tr><th>题号</th><th>分值</th><th>正确答案</th><th>你的答案</th><th>你的得分</th><th>班级平均得分率</th></tr>')
        var preClassScore = TestAnalyze.questionAnalyzeList;
        for (var i = 0; i < preClassScore.length; i++) {
          $('#tableMonth').append('<tr><td>' + preClassScore[i].number + '</td><td>' + preClassScore[i].score + '</td><td>' + preClassScore[i].rightAnswer + '</td><td>' + preClassScore[i].studentAnswer + '</td><td>' + preClassScore[i].studentScore + '</td><td>' + (preClassScore[i].classAvgScoreRate * 100).toFixed(1) + '%</td></tr> ');
        }
      })();
    } catch (err) {
    }
    ...
    (function () {
      ...
      $('#subility_anali').html('<tr><th>学科能力和思想方法</th><th>本次测试涉及分值</th><th>你的得分</th><th>班级平均得分率</th></tr>');
      if (subilityAnalizAry && subilityAnalizAry.length > 0) {
        for (var i = 0; i < subilityAnalizAry.length; i++) {
          $('#subility_anali').append('<tr><td>' + subilityAnalizAry[i].subjectAbilityName + '</td><td>' + subilityAnalizAry[i].subjectAbilityScore + '</td><td>' + subilityAnalizAry[i].studentSubjectAbilityScore + '</td><td>' + (subilityAnalizAry[i].classSubjectAbilityScoreRate * 100).toFixed(1) + '%</td></tr>')
        }
      }
    })()
    ...
    try {
      (function () {
        $('#testDate>span:nth-child(1)').html('<b class="testDateSpan">' + deelwithscore(PersonReportData.questionData.studentScore, 1) + '</b>');
        $('#testDate>span:nth-child(2)').html('');
        $('#testDate>span:nth-child(3)').html('');
      })();
    } catch (err) {
    }
    ...
    try {
      (function () {
        $('#pId_two').hide();
        $('#pId_three>h3>b').html('2、本次测试试题分析');
        $('#tableMonth').html('<tr><th>题号</th><th>分值</th><th>正确答案</th><th>你的答案</th><th>你的得分</th><th>试题难度</th></tr>')
        var TestAnalyze = PersonReportData.thisTestQuestionAnalyze;
        var preClassScore = TestAnalyze.questionAnalyzeList;
        for (var i = 0; i < preClassScore.length; i++) {
          $('#tableMonth').append('<tr><td>' + preClassScore[i].number + '</td><td>' + preClassScore[i].score + '</td><td>' + preClassScore[i].rightAnswer + '</td><td>' + preClassScore[i].studentAnswer + '</td><td>' + preClassScore[i].studentScore + '</td><td>' + preClassScore[i].difficult + '</td></tr> ');
        }
    ...
    try {
      (function () {
        $('#testWeek_retail>h3>b').html('3、本次考试知识点分析');
        $('#pId_five>h3>b').html('4、本次测试学科能力和思想方法分析');
        $('#subility_anali').html('<tr><th>学科能力和思想方法</th><th>本次测试涉及分值</th><th>你的得分</th><th>得分率 </th></tr>');
        var subilityAnalizAry = PersonReportData.thisTestSubjectAbilityAndThoughtMethodAnaylze;
        if (subilityAnalizAry.length > 0) {
          for (var i = 0; i < subilityAnalizAry.length; i++) {
            $('#subility_anali').append('<tr><td>' + subilityAnalizAry[i].subjectAbilityName + '</td><td>' + subilityAnalizAry[i].subjectAbilityScore + '</td><td>' + subilityAnalizAry[i].studentSubjectAbilityScore + '</td><td>' + subilityAnalizAry[i].studentSubjectAbilityScoreRate * 100 + '%</td></tr>')
          }
        } else {
...
function delayload(contentType, perId) {
  ...
  function deelwithFace() {
    ...
    ospanWeek[0].innerHTML = '第 ' + PersonReportData.cover.testWeek.index + ' 教学周' + '(' + PersonReportData.cover.testWeek.learningtime + ')'; //个人信息
  ...
  function deelWithYemei() {
    ...
    for (var i = 0; i < yemei.length; i++) {
      yemei[i].innerHTML = PersonReportData.cover.studentName + "同学" + subjectName + "学情周报" + '(第' + PersonReportData.cover.testWeek.index + '教学周' + PersonReportData.cover.testWeek.learningtime + ')'
    }
  }
  ...
  function AnalysisTest() {
    ...
    $('#tableMonth').append('<tr><th>题号</th><th>分值</th><th>正确答案</th><th>你的答案</th><th>你的得分</th><th>班级平均得分率</th><th>年级平均得分率</th></tr>')
    var preClassScore = TestAnalyze.questionAnalyzeList;
    for (var i = 0; i < preClassScore.length; i++) {
      $('#tableMonth').append('<tr><td>' + preClassScore[i].number + '</td><td>' + preClassScore[i].score + '</td><td>' + preClassScore[i].rightAnswer + '</td><td>' + preClassScore[i].studentAnswer + '</td><td>' + preClassScore[i].studentScore + '</td><td>' + (preClassScore[i].classAvgScoreRate * 100).toFixed(1) + '%</td><td>' + (preClassScore[i].gradeAvgScoreRate * 100).toFixed(1) + '%</td></tr> ');
    }
  ...
  function deelknowledge() {
    ...
    for (var i = 0; i < WeakPiont.length; i++) {
      ...
      knowledgeExplain.innerHTML = "知识点讲解" + ":" + formatQuestionText1(WeakPiont[i].pointExplain);
      knowledgeExplain.className = "fontChange";
  ...
  function subility_analize() {
    ...
    if (typeof subilityAnalizAry == 'undefined') {
      ...
      if (subilityAnalizAry.length > 0) {
        for (var i = 0; i < subilityAnalizAry.length; i++) {
          $('#subility_anali').html('<tr><th>学科能力和思想方法</th><th>本次测试涉及分值</th><th>你的得分</th><th>班级平均得分率</th><th>年级平均得分率</th></tr>');
          $('#subility_anali').append('<tr><td>' + subilityAnalizAry[i].subjectAbilityName + '</td><td>' + subilityAnalizAry[i].subjectAbilityScore + '</td><td>' + subilityAnalizAry[i].studentSubjectAbilityScore + '</td><td>' + (subilityAnalizAry[i].classSubjectAbilityScoreRate * 100).toFixed(1) + '%</td><td>' + (subilityAnalizAry[i].gradeSubjectAbilityScoreRate * 100).toFixed(1) + '%</td></tr>')
        }
      } else {
        $('#subility_analize').hide();
      }
...
function bindUrlTOa() {
  var pdfUrl = "http://115.29.220.75:8080/WKPDFReport/import?type=AI&key=id&value=" + reportId + "&filename=" + "RS" + reportId
