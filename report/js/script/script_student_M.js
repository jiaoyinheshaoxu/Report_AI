...
function delayload(contentType,perId){
    ...
    function deelwithFace() {
       ...
       if(subjectName=='初中数学'||subjectName=='高中数学'||subjectName=='高中文科数学'||subjectName=='高中理科数学'||subjectName=='小学数学'){
        ...
        oh1[1].querySelectorAll("span")[0].innerHTML=examTypeText+"分析报告";
        oh1[1].querySelectorAll("span")[1].innerHTML="（"+subjectName+"）";
...
//      faceDetail[0].innerHTML = PersonReportData.cover.code;
    ...
    function deelWithYemei(){
        ...
        for(var i=0;i<yemei.length;i++){
            yemei[i].innerHTML=PersonReportData.cover.studentName+"同学"+subjectName+examTypeText+"分析报告"+'('+PersonReportData.cover.testTime+')'
        }
    }
    try {
        deelWithYemei()
...
//_______________________________________________________________________________________学科能力图谱结构变化新旧判断
    ...
    function AnalysisTest(){
        ...
        var preClassScore=TestAnalyze.questionAnalyzeList;
            ...
            $('#tableMonth').append('<tr><td>'+preClassScore[i].number+'</td><td>'+preClassScore[i].score+'</td><td>'+preClassScore[i].rightAnswer+'</td><td>'+preClassScore[i].studentAnswer+'</td><td>'+preClassScore[i].studentScore+'</td><td>'+(preClassScore[i].classAvgScoreRate*100).toFixed(0)+'%</td><td>'+(preClassScore[i].gradeAvgScoreRate*100).toFixed(0)+'%</td></tr> ');
        }
        var testDate1=document.getElementById("testDate1");
        var testB=testDate1.querySelectorAll("b");
        for(var j=0;j<testB.length;j++){
            testB[j].className+="testDateSpan "
        }
        testB[2].innerHTML=TestAnalyze.rightQuestionNum;
    }
    try{
        AnalysisTest();
    }catch(err){
...
function bindUrlTOa(){
    ...
    var pdfUrl = '';
    if(stuName){
        pdfUrl="http://115.29.220.75:8080/WKPDFReport/import?type="+7+"&key=id&value="+	     reportId+"&filename="+"RS"+reportId + '(' + stuName + ')';
    } else {
        pdfUrl="http://115.29.220.75:8080/WKPDFReport/import?type="+7+"&key=id&value="+	     reportId+"&filename="+"RS"+reportId;
    }
    loadPdf.href=pdfUrl;
