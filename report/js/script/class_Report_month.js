...
function checkGrade(){
    if(reportType!==2){
        //一、本次考试分析
        $('.AverageScore').show();
        $('.part_12>h3>b').html('班级得分分布情况');//2、班级得分分布情况
        $('.part_12_1').append(
            '<div>优秀率：'+(ClassReportData.thisTestAnalysis.excellentRate*10000/100)+'% </div>'+
            '<div>及格率：'+(ClassReportData.thisTestAnalysis.passRate*10000/100)+'% </div>'+
            '<div>低分率：'+(ClassReportData.thisTestAnalysis.lowRate*10000/100)+'% </div>'
        );
        //try{
        (function () {
            var preClass=ClassReportData.thisTestAnalysis.classScoresection;
...
//  judgeJurisdiction();
    ...
    function deelwithFace() {
        ...
        ospanWeek[0].innerHTML=ClassReportData.cover.createtime;
        //个人信息
        var personalInfo=document.getElementById("personalInfo");
        var oh1=personalInfo.querySelectorAll("h1");
        oh1[0].querySelector("span").innerHTML=ClassReportData.cover.classname;
        oh1[1].querySelector("span").innerHTML="月考报告（"+ClassReportData.cover.subjectname+"）";
        //学号，班级
        var faceDetail = document.getElementsByClassName("faceDetail");
//      faceDetail[0].innerHTML =ClassReportData.covermap.classno;
        faceDetail[0].innerHTML = ClassReportData.cover.schoolname;
        faceDetail[1].innerHTML = ClassReportData.cover.gradename;
        faceDetail[2].innerHTML = ClassReportData.cover.materialname;
    }
    deelwithFace();
    try {
    ...
    function deelWithYemei(){
        ...
        for(var i=0;i<yemei.length;i++){
            yemei[i].innerHTML=ClassReportData.cover.classname+ClassReportData.cover.subjectname+"考试分析报告"+"( "+ClassReportData.cover.createtime+")"
        }
    }
    try {testDateSpan
        deelWithYemei()
    ...
    function deelwithPage2() {
        ...
        function anserwerDate() {
            ...
            testSPan[0].innerHTML= ClassReportData.summaryData.studenttotal;
            testSPan[1].innerHTML= ClassReportData.summaryData.completetotal;
            //app
            var qData=ClassReportData.summaryData;
        ...
        function classicalErr(){
            var third=ClassReportData.summaryData;
        ...
        //3、班级薄弱点
        function weakPoint(){
            var weaklist0= ClassReportData.summaryData.weaklist,weaklist=[];
            var weakAbilitylist0=ClassReportData.summaryData.weakAbilitylist,weakAbilitylist=[];
			...
			if(weaklist0.length>0){
			    $('#weakPoint li:nth-child(1)').append('班级薄弱知识点: <span class="testDateSpan">'+weaklist.toString()+'</span>等'+weaklist0.length+'个。');
			}else{
			    $('#weakPoint li:nth-child(1)').append('班级薄弱知识点: <span class="testDateSpan">'+weaklist.toString()+'</span>');
			}
			if(weakAbilitylist0.length>0){
				...
				$('#weakPoint li:nth-child(2)').append('班级薄弱能力和思想方法: <span class="testDateSpan">'+weakAbilitylist.toString()+'</span>');
    ...
    function weektest(){
        ...
        oScore.innerHTML=deelwithscore(ClassReportData.thisTestAnalysis.avgScore,1);
        //rank
        var oRank=document.getElementsByClassName("weekTestRank");
        oRank[0].innerHTML=deelwithscore(ClassReportData.thisTestAnalysis.gradeAvgScore,1)+"分";
        oRank[1].innerHTML="第"+ClassReportData.thisTestAnalysis.graderank+"名";
        oRank[2].innerHTML=ClassReportData.thisTestAnalysis.studenttotal;
        oRank[3].innerHTML=ClassReportData.thisTestAnalysis.completetotal;
        oRank[4].innerHTML=ClassReportData.thisTestAnalysis.difficulty;
        oRank[5].innerHTML=ClassReportData.thisTestAnalysis.distinction_degree;
    ...
    (function () {
        ...
        $('.part_12>h3>b').html('班级得分分布情况');//2、班级得分分布情况
        $('.part_12_1').append(
            '<div>高分段学生平均分 '+ClassReportData.thisTestAnalysis.highAvg+' 分</div>'+
            '<div>低分段学生平均分 '+ClassReportData.thisTestAnalysis.lowAvg+' 分</div>'
        );
        var preClass=ClassReportData.thisTestAnalysis.classScoresection;
        var classAry1={},classObj=[],classAry3=[];
        for(var i=0;i<preClass.length;i++){
            classAry1={};
            classAry1.name=preClass[i].key;
            classAry1.y=preClass[i].value;
            classAry3.unshift(classAry1)
        }
    ...
    try{
        (function () {
            var ary=ClassReportData.questionAnalysis.distributionList;
            var ary0=[];
            for(var i=0;i<ary.length;i++){
                ary0.push({x:ary[i].scoreRate,y:ary[i].distributionRate,z:0.5,name:ary[i].sequence+"题"})
            }

            var colors = ['#2090fa', '#f07b18', '#38db34'];
            ...
            //x 坐标名称
            var ary0=ClassReportData.questionAnalysis.distributionList;
            var aryX=[],aryY1=[],aryY2=[];
            for(var i=0;i<ary0.length;i++){
                aryX.push(ary0[i].sequence+"题");
                aryY1.push(ary0[i].scoreRate);
                aryY2.push(ary0[i].distributionRate);
            }
    ...
    function testAnalycis2(){
        var ary=ClassReportData.questionAnalysis.paperQuestionInfoList;
        for(var i=0;i<ary.length;i++){
            ary[i].leixing="";
            switch(ary[i].qtype){
    ...
    function KnowledgeAnalyze(){
        var obj=ClassReportData.pointAnalysis;
        ...
        var ary1=ClassReportData.pointAnalysis.classweak;
        var ary2=ClassReportData.pointAnalysis.highweak;
		...
		if(ary1.length>0){
			    ...
			    $('#weakKnowledge li:nth-child(1)' ).append('班级薄弱知识点有:<span class="testDateSpan">'+aryClass+'</span>');
		...
		if(ary2.length>0){
			    ...
			    $('#weakKnowledge li:nth-child(2)' ).append('高分段学生的薄弱知识点有:<span class="testDateSpan">'+aryHigh+'</span>');
		}
    }
    ...
    try{
        (function () {
            ...
            //x 坐标名称
            var ary0=ClassReportData.pointAnalysis.pointDistribution;
            var aryX=[],aryY1=[],aryY2=[];
            for(var i=0;i<ary0.length;i++){
                aryX.push(ary0[i].pointName);
                aryY1.push(ary0[i].pointScoreRate);
                aryY2.push(ary0[i].distributionRate);
            }
    ...
    function KnowledgeAnalyze2(){
        var ary=ClassReportData.pointAnalysis.unitList;
		var fullscore=ClassReportData.summaryData.score;
        for(var i=0;i<ary.length;i++){
            $('#KnowledgeAnalyze2').append('<tr>'+'<td>'+ary[i].unitName+'</td>'+'<td>'+''+'</td><td>'+''+'</td><td>'+(ary[i].frequency/100000*fullscore).toFixed(1)+'</td><td>'+ary[i].score+'</td><td>'+''+'</td><td>'+'</td><td>'+''+'</td><td>'+''+'</td></tr>');
            ...
            for(var j= 0,ary2=ary21;j<ary2.length;j++){
                $('#KnowledgeAnalyze2').append('<tr>'+'<td>'+''+'</td>'+'<td>'+ary2[j].chapter+'.'+ary2[j].name+'</td><td>'+''+'</td><td>'+(ary2[j].frequency/100000*fullscore).toFixed(1)+'</td><td>'+ary2[j].score+'</td><td>'+'</td><td>'+''+'</td><td>'+''+'</td><td>'+''+'</td></tr>');
    ...
    function subAbilityAnalyze(){
        var ary=ClassReportData.subjectPower.abilityInfoList;
        for(var i=0;i<ary.length;i++){
    ...
    function improveAbility(){
        var ary=ClassReportData.subjectPower.weakAbility;
        if(ary.length==0){
...
//
    ...
    function erranswerList(){
        ...
        var datalist0=ClassReportData.classErrorQuestion;
...
//        ocircle.style.height="120px";
            ...
            var rate0= list[i].unitGradeArate*10000/100;
    ...
    function creatSvgCircle1(list,odivId){
        ...
        for(var i=0;i<list.length;i++){
            ...
            rate0= list[i].unitGradeArate*10000/100;
            rate=rate0.toFixed(1)-0;
            ocircle="ocircle"+i;
...
function bindUrlTOa(){
    var pdfUrl="http://115.29.220.75:8080/WKPDFReport/import?type="+14+"&key=id&value="+	     reportId+"&filename="+"RC"+reportId;
    loadPdf.href=pdfUrl;
